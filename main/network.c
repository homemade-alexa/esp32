...
static const char *TAG = "ALEXA/NETWORK";
...
void set_hostname(esp_mac_type_t emt)
{
    ...
    snprintf(hostname, HOSTNAME_SIZE, "alexa-%02x%02x%02x%02x%02x%02x", mac[0], mac[1], mac[2], mac[3], mac[4],
             mac[5]);
...
esp_err_t init_wifi(const char *psk, const char *ssid)
{
    ...
    // Start wifi
    ui_display_text(NULL, NULL, "Łączę z Wi-Fi...", NULL, NULL);
